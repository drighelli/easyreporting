...
```{r}
er$mkdTitle(title="Code Chunks", level=1)
...
```{r}
er$mkdTitle(title="Manual code chunk", level=2)

er$mkdCodeChunkSt()
variable <- 1
er$mkdVariableAssignment("variable", "variable", show=TRUE)
er$mkdCodeChunkEnd()
```
...
er$mkdCodeChunkSt(optionsList=optList)
er$mkdCodeChunkEnd()
```
...
er$mkdCodeChunkSt(optionsList=optList, sourceFilesList=RFilesList)
er$mkdGeneralMsg(message="v <- fakeFunction(10)")
er$mkdCodeChunkEnd()
```
...
```{r}
er$mkdCodeChunkComplete(message="v <- fakeFunction(11)")
```
...
er$mkdCodeChunkCommented(commentMsg="This is the comment of the following code chunk",
                    message="v <- fakeFunction(12)",
                    optionsList=optList,
